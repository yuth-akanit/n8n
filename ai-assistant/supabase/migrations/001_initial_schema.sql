...
create policy "ai_messages_select" on ai_messages
  for select using (
    exists (select 1 from ai_sessions s where s.id = session_id and is_workspace_member(s.workspace_id))
  );
...
create policy "seo_audits_select" on seo_audits
  for select using (
    exists (select 1 from seo_sites s where s.id = site_id and is_workspace_member(s.workspace_id))
  );
create policy "seo_audit_pages_select" on seo_audit_pages
  for select using (
    exists (
      ...
      where a.id = audit_id and is_workspace_member(s.workspace_id)
    )
  );
