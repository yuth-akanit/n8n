...
export function PlanClient({ projectId, workspaceId }: Props) {
  ...
  async function handleGenerate(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/ai/projects', {
        ...
        body: JSON.stringify({ workspaceId, projectId, goal, scope }),
      })
  ...
  async function handleSavePlan() {
    ...
    try {
      const res = await fetch(`/api/projects/${projectId}/plan`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ milestones: result.milestones, summary: result.summary }),
      })
      const data = await res.json()
  ...
  return (
    <div>
      <PageHeader
        ...
        action={<Link href={`/dashboard/projects/${projectId}`} className="btn-secondary">← Project</Link>}
      />
